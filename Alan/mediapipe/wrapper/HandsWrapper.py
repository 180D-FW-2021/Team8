...
I think we'll be able to integrate this more seamlessly with the game logic than with keyboard input. Need to clarify info transfer method. May avoid writing to .csv in the first place!
'''
			...
			if results.multi_hand_landmarks:
				...
				#id pulling code below from https://arkalsekar.medium.com/how-to-get-all-the-co-ordinates-of-hand-using-mediapipe-hand-solutions-ac7e2742f702
					for ids, landmrk in enumerate(hand_landmarks.landmark):
						# print(ids, landmrk)
						if ids == 0: #wrist node. But for which hand if 2 are onscreen?
							f = open('wrist_single.csv', 'w', newline='')
							writer = csv.writer(f)
							cx, cy = landmrk.x, landmrk.y
							writer.writerow([cx, cy])
							f.close()
